...
async function cargarModeloVisual() {
    ...
    if (!window.tf || !window.mobilenet) {
        throw new Error("El módulo de búsqueda visual no está disponible.");
    }
    modeloVisual = await window.mobilenet.load({ version: 2, alpha: 1.0 });
    return modeloVisual;
}
...
async function imagenDesdeUrl(url) {
    return await new Promise((resolve, reject) => {
        ...
        img.onerror = () => {
            ...
            img2.onerror = () => reject(new Error("No se pudo cargar una foto."));
            img2.src = url;
        };
        img.src = url;
    });
...
async function obtenerVectorVisual(img, modelo) {
    return await tf.tidy(() => {
        const embedding = modelo.infer(img, true);
...
async function procesarBusquedaFoto(file) {
    ...
    try {
        ...
        if (estado) estado.textContent = "🔎 Comparando con las fotos de tus peluches...";
        ...
        for (const p of peluches) {
            ...
            for (const foto of fotos.slice(0, 4)) {
                try {
                    const img = await imagenDesdeUrl(foto);
                    const vector = await obtenerVectorVisual(img, modelo);
                    const score = similitudCoseno(consulta, vector);
                    if (score > mejor) mejor = score;
                } catch (e) {
                    console.warn("No se pudo analizar una foto:", e);
                }
            }

            if (mejor >= 0) candidatos.push({ p, score: mejor });
        }
        ...
        if (!top.length) {
            if (estado) estado.textContent = "No hay peluches con fotos suficientes para comparar.";
            return;
        }

        if (estado) estado.textContent = `✨ Encontré ${top.length} coincidencia${top.length === 1 ? "" : "s"}. Revisa la similitud antes de elegir.`;
        ...
        resultadosEl.innerHTML = top.map(({p, score}) => {
            ...
            return `
                <button type="button" class="resultado-foto" data-foto-id="${p.id}">
                    <img src="${foto}" alt="">
                    <span class="resultado-foto-info">
                        <strong>${escapeHtml(p.nombre || "Peluche sin nombre")}</strong>
                        <small>${escapeHtml(p.codigo || p.codigoInterno || "Sin código interno")}</small>
        ...
        console.error(error);
        if (estado) estado.textContent = "❌ No pude realizar la búsqueda visual. Revisa tu conexión e inténtalo de nuevo.";
    } finally {
